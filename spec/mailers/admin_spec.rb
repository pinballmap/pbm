...
RSpec.describe AdminMailer, type: :mailer do
  ...
  describe 'send_daily_digest_region' do
    it 'should send a daily region digest email' do
      ...
      score_item    = { location_name: 'Foo Bar', location_id: 1, machine_name: 'Centaur', high_score: 1_000_000, user_name: 'dave' }
      email_subject = "Pinball Map - Daily activity digest (Portland) - #{(Date.today - 1.day).strftime('%m/%d/%Y')}"

      email = AdminMailer.with(email_to: 'foo@bar.com', email_subject: email_subject, region_name: 'Portland', machine_comments: [ comment_item ], machine_removals: [ removal_item ], pictures_added: [ picture_item ], high_scores: [ score_item ]).send_daily_digest_region
  ...
  describe 'send_daily_digest_global' do
    it 'should send a daily global digest email' do
      ...
      email = AdminMailer.with(user: 'foo@bar.com', machine_comments: [ comment_item ], machine_removals: [], pictures_added: [], high_scores: [], location_metadata: [], remove_and_readd: []).send_daily_digest_global
