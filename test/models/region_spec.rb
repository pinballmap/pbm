...
describe Region do
  ...
  describe '#generate_weekly_global_email_body' do
    it 'should generate a string containing metrics about global locations' do
      ...
      expect(Region.generate_weekly_global_email_body[:machineless_locations]).must_equal [ 'Another Region Location (Portland, OR)', 'Empty Location (Troy, OR)', 'Another Empty Location (Rochester, OR)' ]
      expect(Region.generate_weekly_global_email_body[:suggested_locations_count]).must_equal 2
      expect(Region.generate_weekly_global_email_body[:locations_added_count]).must_equal 3
      expect(Region.generate_weekly_global_email_body[:locations_deleted_count]).must_equal 3
      expect(Region.generate_weekly_global_email_body[:machine_comments_count]).must_equal 2
      expect(Region.generate_weekly_global_email_body[:machines_added_count]).must_equal 4
      expect(Region.generate_weekly_global_email_body[:machines_removed_count]).must_equal 2
      expect(Region.generate_weekly_global_email_body[:pictures_added_count]).must_equal 2
      expect(Region.generate_weekly_global_email_body[:scores_added_count]).must_equal 2
    end
  end
  ...
  describe '#generate_weekly_admin_email_body' do
    it 'should generate a string containing metrics about the region condition' do
      ...
      expect(@region.generate_weekly_admin_email_body[:machineless_locations]).must_equal [ 'Empty Location (Troy, OR)', 'Another Empty Location (Rochester, OR)', 'Test Location Name (Portland, OR)' ]
      ...
      expect(@region.generate_weekly_admin_email_body[:machines_added_count]).must_equal 3
      ...
      expect(@region.generate_weekly_admin_email_body[:events_count]).must_equal 5
      expect(@region.generate_weekly_admin_email_body[:pictures_added_count]).must_equal 2
      expect(@region.generate_weekly_admin_email_body[:scores_added_count]).must_equal 2
