...
Given /^I fill in a score of "([^"]*)" and rank "([^"]*)"$/ do |score, rank|
  fill_in("score", :with => score)
  select(rank, :from => 'rank')
end

Then /^"([^"]*)"'s "([^"]*)" should have a score with score "([^"]*)" and rank "([^"]*)"$/ do |location_name, machine_name, score, rank|
...
Given /^a high score exists for location "([^"]*)"'s "([^"]*)" with initials "([^"]*)" and score "([^"]*)" and rank "([^"]*)"$/ do |location_name, machine_name, initials, score, rank|
  ...
  Factory.create(:machine_score_xref, :user => user, :location_machine_xref => lmx, :score => score, :rank => MachineScoreXref::ENGLISH_SCORES.key(rank))
end
