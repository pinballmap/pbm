...
task send_weekly_admin_digest: :environment do
  Region.all.each do |r|
    ...
    AdminMailer.with(email_to: email_to, email_subject: email_subject, region_name: email_body[:full_name], machines_count: email_body[:machines_count], locations_count: email_body[:locations_count], events_count: email_body[:events_count], contact_messages_count: email_body[:contact_messages_count], machineless_locations: email_body[:machineless_locations], suggested_locations: email_body[:suggested_locations], suggested_locations_count: email_body[:suggested_locations_count], locations_added_count: email_body[:locations_added_count], locations_deleted_count: email_body[:locations_deleted_count], machine_comments_count: email_body[:machine_comments_count], machines_added_count: email_body[:machines_added_count], machines_removed_count: email_body[:machines_removed_count], pictures_added_count: email_body[:pictures_added_count]).send_weekly_admin_digest.deliver_later
    sleep(8)
  end
