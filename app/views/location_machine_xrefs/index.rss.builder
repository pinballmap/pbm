  ...
  xml.channel do
    ...
    @lmxs.each do |lmx|
      ...
      xml.item do
        ...
        xml.link [request.protocol, request.host_with_port, location.region ? region_homepage_path(location.region.name.downcase) : '/regionless', "/?by_location_id=#{location.id}"].join('')
