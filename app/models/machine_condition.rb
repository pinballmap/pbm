class MachineCondition < ApplicationRecord
  ...
  def create_user_submission
    ...
    UserSubmission.create(region_id: location.region_id, location: location, machine: machine, submission_type: UserSubmission::NEW_CONDITION_TYPE, submission: "#{user_info} commented on #{machine.name} at #{location.name} (#{location.city}). They said: #{comment}", user: user)
  end
