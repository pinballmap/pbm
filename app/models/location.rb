class Location < ApplicationRecord
  ...
  def confirm(user)
    ...
    UserSubmission.create(user_name: user.nil? ? nil : user.username, location_name: name, region_id: region&.id, location: self, submission_type: UserSubmission::CONFIRM_LOCATION_TYPE, submission: "#{user ? user.username : 'Someone'} confirmed the lineup at #{name} in #{city}", user: user)
