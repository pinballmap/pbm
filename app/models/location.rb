class Location < ApplicationRecord
  ...
  def recent_activity
    UserSubmission.where.not(submission_type: 'location_metadata', location_id: self).where.not(submission_type: 'ic_toggle', location_id: self).where.not(submission_type: 'delete_location', location_id: self).where(location_id: self, created_at: '2019-05-03T07:00:00.00-07:00'..Date.today.end_of_day).order('created_at DESC')
  end
