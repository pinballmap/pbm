  ...
  module V1
    class UserSubmissionsController < InheritedResources::Base
      ...
      def list_within_range
        ...
        if params[:submission_type]
          user_submissions = UserSubmission.where.not(lat: nil).where(created_at: min_date_of_submission..Date.today.end_of_day, submission_type: params[:submission_type]).near([params[:lat], params[:lon]], max_distance)
        else
          user_submissions = UserSubmission.where.not(lat: nil).where(created_at: min_date_of_submission..Date.today.end_of_day).near([params[:lat], params[:lon]], max_distance)
        end
