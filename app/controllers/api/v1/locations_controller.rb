module Api
  module V1
    class LocationsController < BaseController
      ...
      has_scope :by_location_name, :by_machine_name, :by_city_id, :by_city_no_state, :by_at_least_n_machines, :by_at_least_n_machines_city, :by_at_least_n_machines_zone, :by_at_least_n_machines_type, :region, :by_is_stern_army, :regionless_only, :by_ic_active, :by_machine_year_gte, :by_machine_year_lte, :by_payment_type
      has_scope :by_type_id, :by_location_id, :by_machine_id, :by_zone_id, :by_operator_id, :by_machine_single_id, :by_machine_group_id, :by_ipdb_id, :by_opdb_id, :manufacturer, :by_machine_type, :by_machine_display, :by_machine_id_ic, :by_machine_single_id_ic, :by_machine_year, :by_country, :by_state_name, :by_state_id, :by_all_ages, type: :array
      ...
      param :location_all_ages, String, desc: "Whether the location is all ages. One of 'Yes', 'At Times', and 'No'", required: false
      ...
      param :by_all_ages, String, desc: "Send only all ages locations. Locations tagged 'Yes' or 'At Times' are both included. 'No' for not all ages. Additional filtering required for this endpoint.", required: false
      param :by_payment_type, String, desc: "Send only free play locations. Accepted values are 'Free Play' and 'No' (meaning not Free Play). Additional filtering required for this endpoint.", required: false
      formats [ "json" ]
      def index
      ...
      param :by_all_ages, String, desc: "Send only all ages locations. Locations tagged 'Yes' or 'At Times' are both included. 'No' for not all ages.", required: false
      param :by_payment_type, String, desc: "Send only free play locations. Accepted values are 'Free Play' and 'No' (meaning not Free Play).", required: false
      formats [ "json" ]
      ...
      def closest_by_address
        ...
        location_details = [ location_machine_xrefs: { include: { machine: { methods: :machine_group_id, except: params[:no_details] ? %i[is_active created_at updated_at] : nil } }, except: params[:no_details] ? %i[created_at updated_at user_id] : nil } ]
      ...
      def show
        ...
        if params[:no_details] == "1"
          ...
          if params[:user_id].present?
            life_list_machine_ids = UserMachineXref.where(user_id: params[:user_id], machine_id: location.location_machine_xrefs.map(&:machine_id)).pluck(:machine_id).to_set
